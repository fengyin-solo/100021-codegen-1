  ...
  <section class="page" data-module="vfx">
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr v-if="!rows.length">
          <td :colspan="columns.length + 1" class="empty-state">暂无特效制作数据，可先登记特效镜头</td>
        </tr>
      </tbody>
    </table>
...
const stats = [{"label": "制作中镜头", "value": 0}, {"label": "待审核镜头", "value": 0}, {"label": "已完成镜头", "value": 0}]
...
async function runAction(action: string, row: Row) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('特效制作动作未生效，请稍后重试')
    }
    await reload()
  } catch (error) {
