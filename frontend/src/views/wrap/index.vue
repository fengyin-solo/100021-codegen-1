<template>
  <section class="page" data-module="wrap">
    <header class="page-head">
      <div>
        <h2>杀青结算管理</h2>
        <p class="page-desc">维护结算单，围绕结算单号、结算对象、结算周期、应结金额做登记、筛选与状态流转。</p>
      </div>
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr v-if="!rows.length">
          <td :colspan="columns.length + 1" class="empty-state">暂无杀青结算数据，可先登记结算单</td>
        </tr>
      </tbody>
    </table>

    <footer class="page-foot">
      <span>共 {{ total }} 条杀青结算记录</span>
...
async function runAction(action: string, row: Row) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('杀青结算动作未生效，请稍后重试')
    }
    await reload()
  } catch (error) {
