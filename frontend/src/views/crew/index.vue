<template>
  <section class="page" data-module="crew">
    <header class="page-head">
      <div>
        <h2>剧组人员管理</h2>
        <p class="page-desc">维护剧组成员，围绕成员编号、姓名、岗位职务、所属组别做登记、筛选与状态流转。</p>
      </div>
      <div class="page-actions">
        <button class="btn primary" type="button" @click="openCreate">登记剧组成员</button>
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr v-if="!rows.length">
          <td :colspan="columns.length + 1" class="empty-state">暂无剧组人员数据，可先登记剧组成员</td>
        </tr>
      </tbody>
    </table>
...
const stats = [{"label": "在组人数", "value": 0}, {"label": "待进场人员", "value": 0}, {"label": "今日请假", "value": 0}]
...
function openCreate() {
  errorMessage.value = '剧组成员登记入口尚未接入审批流'
}
...
async function runAction(action: string, row: Row) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('剧组人员动作未生效，请稍后重试')
    }
    await reload()
  } catch (error) {
